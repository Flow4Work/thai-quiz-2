  ...
  return (
    <div className="px-5 pb-10 pt-10">
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <div className="text-sm font-semibold text-[#60A5FA]">{doneCount}/{CATEGORIES.length}</div>
      </div>
...
export default function App() {
  const [completed, setCompleted] = useState<Record<CategoryId, boolean>>(() => {
    const init = Object.fromEntries(CATEGORIES.map((c) => [c.id, false]));
    return init as Record<CategoryId, boolean>;
  });
